        ...
        if let lh = handDetection.latestAnchors.leftHand{
            ...
            if afterSnap(lefthand: lh){
                ...
                if time_tick - last_presnap < 10{
                    ...
                    fire = true
//                    fireballModel.myball.addChild(FireViewModel.fireballAnimated!)
                }
            }
...
extension simd_float4 {
    var xyz: simd_float3 { simd_float3(x: x, y: y, z: z) }
}
...
extension HandAnchor {
    func isPreSnap() -> Bool {
        ...
        let lefthandanchor = originFromAnchorTransform
    ...
    func isPointing() -> simd_float3? {
        guard
            let indextip  = handSkeleton?.joint(.indexFingerTip),
            let indexknuckle = handSkeleton?.joint(.indexFingerKnuckle),
            let indexintertip = handSkeleton?.joint(.indexFingerIntermediateTip)
        else {
            return nil
        }
        let lefthandanchor = originFromAnchorTransform
        let indextiploc = getlocation(jointloc: lefthandanchor, parentloc: indextip.anchorFromJointTransform)
        let indexknuckleloc = getlocation(jointloc: lefthandanchor, parentloc: indexknuckle.anchorFromJointTransform)
        let indexintertiploc = getlocation(jointloc: lefthandanchor, parentloc: indexintertip.anchorFromJointTransform)
        
        if abs(indextiploc.y - indexknuckleloc.y) < 0.01,
           abs(indextiploc.y - indexintertiploc.y) < 0.01 {
            return (indextiploc - indexintertiploc).xyz
        }
        return nil
    }
